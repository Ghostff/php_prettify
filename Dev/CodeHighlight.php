...
class CodeHighlight extends Regex
{
    ...
    */ 
    private static function MatchConst($code)
    {
        ...
        $const_num = preg_split(
			'/[\.\+\-\*\&\%\@\!\,\)\;\:\=\<\>\[\]\\\]+/',
			$code_line
		);
        foreach ($const_num as $new_constant) {
			
			//matched defined const eg (const CONSTANT)
			$rmv_con = ltrim(trim($new_constant), 'const ');
            if (preg_match('/^[A-Z_]+$/', $rmv_con, $matches)) {
